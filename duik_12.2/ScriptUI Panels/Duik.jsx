...
//fenètre des Wires
	...
	{
		var interpoFenetre = new Window ("palette", traduction(["Interpolations","Interpolations","Interpolations"]), [300,300,522,465]);
	...
	palette.bounds = [300,300,560,665];
		...
		//bouton inter
		var boutonInter = panoanimation.add("iconbutton",[112,2,220,20],dossierIcones + "fr/anim_interp.png");
		boutonInter.onClick = function() { interpoFenetre.show(); } ;
		boutonInter.helpTip = traduction(["Opens the interpolations tool","Ouvre l'outil interpolations","Opens the interpolations tool"]);
		//bouton lentille
...
// fonction pour vérifier que les scripts ont les droits décriture
	...
	if (folderTest.exists) {
		if (isSecurityPrefSet() == true)  IKtools(this);
		else {
			app.settings.saveSetting("duik","version","non");
			IKtools(this);
			}
	} else alert( traduction(["\"Duik Icons\" folder is missing in \"" + Folder.current.parent.fsName + "\r\n\r\nYou can find this folder in the zip archive you've downloaded from Duik's website","Il manque le dossier des icônes \"Duik Icons\" dans \"" + Folder.current.parent.fsName + "\r\n\r\nVous trouverez ce dossier dans l'archive que vous avez téléchargée sur le site de Duik","\"Duik Icons\" folder is missing in \"" +Folder.current.parent.fsName + "\r\n\r\nYou can find this folder in the zip archive you've downloaded from Duik's website"]));
